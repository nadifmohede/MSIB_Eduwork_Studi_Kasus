...
<body>
    ...
    $data_transaksi =mysqli_query($koneksi, "SELECT * FROM transaksi WHERE id = '$id' ");
    ...
    while($transaksi = mysqli_fetch_array($data_transaksi)){
        ...
        $tgl_pinjam = $transaksi['tgl_pinjam'];
    ...
    <div class="container">
        ...
        <table>
            <tr>
                <td><h1 class="text-center" style="padding-top: 40px; padding-bottom: 10px;">EDIT DATA TRANSAKSI</h1></td>
            </tr>
        </table>
        </center>
        ...
        <br><br>
        <form action="edit_transaksi_proses.php?id=<?php echo $transaksi['id']; ?>" method="post">
            ...
            <div class="form-group">
                ...
                <select name="nama_penyewa" class="form-control">
                    ...
                    while ($data=mysqli_fetch_array($sql)) {
                    ?>
                    <option value="<?=$data['nama_penyewa']?>">
                        <?=$data['nama_penyewa']." (ID Penyewa = ".$data['id'].")"?>
                    </option>
                    <?php } ?>
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="id_produk" class="form-control">
                    ...
                    <option value="<?=$data['id']?>">
                        <?=$data['id']." (Nama Produk = ".$data['nama_produk'].")" ?>
                    </option>
                    <?php } ?>
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="nama_produk" class="form-control">
                    ...
                    <option value="<?=$data['nama_produk']?>">
                        <?=$data['nama_produk']." (ID Produk = ".$data['id'].")"?>
                    </option>
                    <?php } ?>
                </select>
            </div>
            <div class="form-group">
                ...
                <input type="text" name="tgl_pinjam" id="date1" value="<?php echo $tgl_pinjam; ?>" class="form-control" placeholder="Masukkan Tanggal Pinjam Barang">
            </div>
            <div class="form-group">
                <label>Tanggal Kembali</label>
                <input type="text" name="tgl_kembali" id="date2" value="<?php echo $tgl_kembali; ?>" class="form-control" placeholder="Masukkan Tanggal Kembali Barang">
            </div>
            <div class="form-group">
                <label>Jaminan</label>
                <select name="jaminan" class="form-control">
                    <option disabled selected>--PILIH SALAH SATU--</option>
                    <option value="KTP" <?= ($jaminan == 'KTP' ? 'selected' : ''); ?>>KTP</option>
                    <option value="SIM" <?= ($jaminan == 'SIM' ? 'selected' : ''); ?>>SIM</option>
                    <option value="KK" <?= ($jaminan == 'KK' ? 'selected' : ''); ?>>Kartu Keluarga</option>
                    <option value="Passport" <?= ($jaminan == 'Passport' ? 'selected' : ''); ?>>Passport</option>
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="pembayaran" class="form-control">
                    <option disabled selected>--PILIH SALAH SATU--</option>
                    <option value="Tunai" <?= ($pembayaran == 'Tunai' ? 'selected' : ''); ?>>Tunai</option>
                    <option value="Transfer" <?= ($pembayaran == 'Transfer' ? 'selected' : ''); ?>>Transfer Bank</option>
                    <option value="EDC" <?= ($pembayaran == 'EDC' ? 'selected' : ''); ?>>Mesin EDC</option>
                </select>
            </div>
            <div class="form-group">
